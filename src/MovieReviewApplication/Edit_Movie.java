...
public class Edit_Movie implements ActionListener{
	...
	LName.setForeground(Color.RED);
     ...
     LDes.setBounds(10, 280, 80, 30);
      ...
      Mframe.add(BUpdate);
      BUpdate.setBounds(450,600 ,100 , 40);
    ...
    @Override
      public void actionPerformed(ActionEvent e){
          if(e.getSource().equals(BSearch)){
           ...
           try{
             Connection c=GetConnection.getconnection();
             String query="update movies set Name = ?,Rating = ?,Trailer = ?,Runtime = ?,Crew = ?,Description = ?,Year =? where Name = ?";
              PreparedStatement p = c.prepareStatement(query);
              p.setString(1, SName);
              p.setFloat(2, FRat);
              p.setString(3, STra);
              ...
              p.setString(6, SDes);
              p.setString(7, SDate);
              p.setString(8, change);
              p.executeUpdate();
              c.close();
              JOptionPane.showMessageDialog(BUpdate, "Movie updated", "Success", JOptionPane.INFORMATION_MESSAGE);
