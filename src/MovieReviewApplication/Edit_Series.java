...
public class Edit_Series implements ActionListener{
    JFrame Sframe;
     ...
     LDes.setBounds(10, 320, 80, 30);
      ...
      Sframe.add(BUpdate);
      BUpdate.setBounds(450,630 ,100 , 40);
    ...
    @Override
      public void actionPerformed(ActionEvent e){
          if(e.getSource().equals(BSearch)){
           ...
           try{
             Connection c=GetConnection.getconnection();
             String query="update series set Name = ?,Ratingyet = ?,Trailer = ?,Seasons = ?,Episodes = ?,Cast = ?,Description = ?,Year =? where Name = ?";
              PreparedStatement p = c.prepareStatement(query);
              p.setString(1, SName);
              p.setFloat(2, FRat);
              p.setString(3, STra);
